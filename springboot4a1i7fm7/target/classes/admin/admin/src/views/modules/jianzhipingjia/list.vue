<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
			...
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评价名称</label>
						<el-input v-model="searchForm.pingjiamingcheng" placeholder="评价名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">岗位名称</label>
						<el-input v-model="searchForm.gangweimingcheng" placeholder="岗位名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="评分信息" prop="pingfenxinxi">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">评分信息</label>
						<el-select clearable v-model="searchForm.pingfenxinxi" placeholder="请选择评分信息" >
							<el-option v-for="(item,index) in pingfenxinxiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"10px 0 10PX","color":"rgba(19,161,230,1)","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jianzhipingjia','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('jianzhipingjia','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('jianzhipingjia','岗位评分统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"border":"1px solid rgba(19,161,230,.2)","width":"100%","padding":"0","borderRadius":"0px","background":"#fff"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="pingjiamingcheng"
						label="评价名称">
						<template slot-scope="scope">
							{{scope.row.pingjiamingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column :resizable='true' :sortable='true' prop="pingjiatupian" width="200" label="评价图片">
						<template slot-scope="scope">
							<div v-if="scope.row.pingjiatupian">
								<img v-if="scope.row.pingjiatupian.substring(0,4)=='http'" :src="scope.row.pingjiatupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.pingjiatupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.pingjiatupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.pingjiatupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('jianzhipingjia','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jianzhipingjia','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								更多
							</el-button>
							<el-button class="edit" v-if=" isAuth('jianzhipingjia','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jianzhipingjia','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="pingfenxinxiChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "jianzhipingjia/group/pingfenxinxi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].pingfenxinxi);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].pingfenxinxi
                    })
                }
                var option = {};
                option = {
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.pingjiamingcheng!='' && this.searchForm.pingjiamingcheng!=undefined){
            params['pingjiamingcheng'] = '%' + this.searchForm.pingjiamingcheng + '%'
          }
           ...
           if(this.searchForm.pingfenxinxi!='' && this.searchForm.pingfenxinxi!=undefined){
            params['pingfenxinxi'] = this.searchForm.pingfenxinxi
          }
    params['sort'] = 'pingjiariqi';
    params['order'] = 'desc';
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          pingjiamingcheng: row.pingjiamingcheng,
          gangweimingcheng: row.gangweimingcheng,
          pingjiatupian: row.pingjiatupian,
          pingfenxinxi: row.pingfenxinxi,
          pingjiaxinxi: row.pingjiaxinxi,
	...
	jianzhipingjiastatusChange(e,row){
		if(row.status==0){
