<template>
	<div class="main-content" :style='{"padding":"20px 30px 30px","margin":"0 auto","color":"#666","borderRadius":"0 0 10px 10px","background":"#fff","width":"calc(100% - 60px)","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 10px 0","margin":"0px","fontSize":"inherit","overflow":"hidden","background":"none"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">薪资待遇</label>
						<el-input v-model="searchForm.xinzidaiyu" placeholder="薪资待遇" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
          ...
          <el-button class="btn18" v-if="isAuth('jianzhixinxi','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
          ...
          <el-button class="btn18" v-if="isAuth('jianzhixinxi','兼职类型统计')" type="success" @click="chartDialog1()">
					...
					<el-table-column :resizable='true' :sortable='true' prop="bangonghuanjing" width="200" label="办公环境">
						<template slot-scope="scope">
							<div v-if="scope.row.bangonghuanjing">
								<img v-if="scope.row.bangonghuanjing.substring(0,4)=='http'" :src="scope.row.bangonghuanjing.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.bangonghuanjing.split(',')[0])">
								<img v-else :src="$base.url+scope.row.bangonghuanjing.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.bangonghuanjing.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
          ...
          <el-table-column :resizable='true' :sortable='true'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('jianzhixinxi','兼职申请')" @click="shenqingxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','您已提交申请！')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('jianzhixinxi','私聊')" type="success" @click="chatClick(scope.row)">
		...
		<shenqingxinxi-cross-add-or-update v-if="shenqingxinxiCrossAddOrUpdateFlag" :parent="this" ref="shenqingxinxiCrossaddOrUpdate"></shenqingxinxi-cross-add-or-update>
    ...
    <el-dialog :title="this.batchIds.length>1?'批量审核':'审核'" :visible.sync="sfshBatchVisiable" width="50%">
      ...
      <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="jianzhileixingChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import shenqingxinxiCrossAddOrUpdate from "../shenqingxinxi/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				shenqingxinxiCrossAddOrUpdateFlag: false,
...
//批量审核
        ...
        this.$refs["shBatchForm"].validate(valid => {
          if(valid){
            this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
              ...
              this.$http({
                url: "jianzhixinxi/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
                method: "post",
                data: this.batchIds
              }).then(({ data }) => {
                if (data && data.code === 0) {
      ...
      getChatList() {
			...
			shenqingxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shenqingxinxiCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shenqingxinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shenqingxinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var jianzhileixingChart1 = echarts.init(document.getElementById("jianzhileixingChart1"),'macarons');
        this.$http({
            url: "jianzhixinxi/group/jianzhileixing",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].jianzhileixing);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].jianzhileixing
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                jianzhileixingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    jianzhileixingChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.xinzidaiyu!='' && this.searchForm.xinzidaiyu!=undefined){
            params['xinzidaiyu'] = '%' + this.searchForm.xinzidaiyu + '%'
          }
